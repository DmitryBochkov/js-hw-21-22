...
var test = {
  ...
  questions: [{ title: 'Какие конструкции для циклов есть в javascript?',
    answers: [{ text: 'Только две: for и while.' }, { text: 'Только одна: for.' }, { text: 'Три: for, while и do...while.' }]
  }, { title: 'В каком случае из перечисленных событие не попадет на обработку javascript?',
    answers: [{ text: 'Если в момент его наступления обрабатывается другое событие.' }, { text: 'Если страничка просматривается локально, т.е offline.' }, { text: 'Только если javascript отключен.' }]
  }, { title: 'Какое событие не вызывается кликом мыши ?',
    answers: [{ text: 'onfocus' }, { text: 'onclick' }, { text: 'onkeydown' }]
  }],
...
var objTest = localStorage.getItem('test');
objTest = JSON.parse(objTest);

// creating the template
var questions = objTest.questions;
var tmpl = _.template(document.getElementById('form-template').innerHTML);
var result = tmpl(objTest);
document.body.insertAdjacentHTML("beforeBegin", result);

// on-click data validation and displaying a modal
var userAnswers = void 0,
    copyUserAnswers = void 0,
    rightAnswers = void 0;

var modalMessage = document.getElementById('modalMessage');
var resetButton = document.getElementById('testResetButon');
...
document.querySelector("button").addEventListener("click", function (event) {
  ...
  function correctUserInput() {
    if (userAnswers.length < objTest.questions.length) {
      ...
      for (var i = 0; i < userAnswers.length - 1; i++) {
  ...
  if (correctUserInput()) {
    var modalRightAnswers = [];
    var modalWrongAnswers = [];
  ...
  function displayModal(messageText) {
    ...
    var btn = document.getElementById("checkResults");
